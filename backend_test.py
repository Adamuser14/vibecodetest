...
class CarRentalSaaSBackendTest:
    ...
    def test_04_create_agency(self):
        ...
        try:
            ...
            print(f"❌ Error in agency creation: {str(e)}")
    ...
    def test_05_get_agencies(self):
        ...
        assert response.status_code == 200, "Get agencies failed with non-200 status code"
        assert "agencies" in response.json(), "Get agencies response missing agencies data"
        assert isinstance(response.json()["agencies"], list), "Agencies data is not a list"
    ...
    def test_08_get_agency_cars(self):
        ...
        assert response.status_code == 200, "Get agency cars failed with non-200 status code"
        assert "cars" in response.json(), "Get agency cars response missing cars data"
        assert isinstance(response.json()["cars"], list), "Cars data is not a list"
    ...
    def test_09_get_agency_bookings(self):
        ...
        assert response.status_code == 200, "Get agency bookings failed with non-200 status code"
        assert "bookings" in response.json(), "Get agency bookings response missing bookings data"
        assert isinstance(response.json()["bookings"], list), "Bookings data is not a list"
